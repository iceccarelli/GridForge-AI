...
/**
 * Spiky AI Load EMS Monitor — streaming edition.
 *
 * The waveform streams across a moving time axis in real time, like a control-
 * room monitor: events arrive at the "now" edge and the stack absorbs them live.
 * It is a REAL-TIME SIMULATION of a deterministic, modeled load — NOT a live GPU
 * telemetry feed (no operator publishes one). On a real deployment this same
 * monitor renders the client's actual telemetry. Price/grid context IS live,
 * pulled from the EPEX + Fraunhofer feeds used across the site.
 *
 * Stack (bottom → top, summing to load):
...
const LEAD = 14; // extra seconds computed off-screen to settle the fuel-cell EMA
const RES = 0.5; // sample resolution (s)
const TICK_MS = 150; // wall-clock cadence
const STEP_S = 0.6; // sim-seconds advanced per tick
...
/** Compute the visible window ending at tNow. Deterministic → SSR-safe. */
function computeFrame(tNow: number, loadMW: number, renewPct: number, gen: Gen) {
  ...
  const all = pts.map((p, i) => {
    ...
    const rel = p.t - tNow; // 0 at right edge = now
...
export function LoadSimulator() {
  ...
  // refs so the stream loop reads latest values without restarting every tick
  ...
  useEffect(() => {
    let on = true;
    fetch("/api/market").then((r) => r.json()).then((d) => on && d?.ok && typeof d.current === "number" && setLive((s) => ({ ...s, price: d.current }))).catch(() => {});
    fetch("/api/grid").then((r) => r.json()).then((d) => on && d?.ok && typeof d.renewablePct === "number" && setLive((s) => ({ ...s, renew: d.renewablePct }))).catch(() => {});
    return () => { on = false; };
  }, []);

  // recompute immediately when knobs change (so a paused stream still updates)
  useEffect(() => {
    ...
    <div className="panel p-6 sm:p-8 relative overflow-hidden">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4 mb-5 relative">
        <div>
          ...
          <p className="text-sm text-mute mt-1 max-w-md">
            Pick a workload. Watch the stack catch each event as it streams in.
          </p>
        </div>
        <div className="flex flex-col items-end gap-2 shrink-0">
          <span className="inline-flex items-center gap-2 data text-[11px]">
            <span className={`w-2 h-2 rounded-full ${streaming ? "bg-verified animate-pulse" : "bg-faint"}`} />
            <span className={streaming ? "text-verified" : "text-faint"}>{streaming ? "STREAMING" : "PAUSED"}</span>
          </span>
          <span className="pill pill-progress">REAL-TIME SIMULATION · MODELED WAVEFORM</span>
        </div>
      </div>
        ...
        {SCENARIOS.map((s) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-[13px] font-semibold border transition ${
              s.id === scenarioId ? "border-power/60 text-power bg-power/10" : "border-line text-mute hover:text-ghost"
            }`}
          >
            {s.label}
          </button>
      ...
      <div className="h-[300px] w-full relative" aria-hidden>
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={series} margin={{ top: 8, right: 10, bottom: 4, left: -16 }}>
            ...
            />
            <Tooltip
              contentStyle={{ background: "#0B1120", border: "1px solid #1E2942", borderRadius: 10, fontFamily: "var(--font-mono), monospace", fontSize: 12 }}
              labelStyle={{ color: "#8A94A6" }}
              labelFormatter={(v) => (Number(v) === 0 ? "now" : `${v}s`)}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mt-3 mb-4">
        ...
        <button
          ...
          className="btn-secondary px-3 py-1.5 rounded-lg text-[13px] inline-flex items-center gap-2 shrink-0"
        >
      ...
      {/* Live metrics — recompute every frame over the visible window */}
      ...
      <div className="mt-5 pt-5 border-t border-line flex flex-wrap items-center justify-between gap-3">
        <p className="text-xs text-faint max-w-xl leading-relaxed">
          Real-time playback of a modeled waveform — no operator publishes live GPU
          telemetry, so on a real deployment this monitor streams your cluster&apos;s
          actual data instead. Price and grid context are live (EPEX · Fraunhofer).
        </p>
